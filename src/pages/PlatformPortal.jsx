...
export default function PlatformPortal() {
  ...
  if (loading) {
    return (
      <div className="fixed inset-0 flex items-center justify-center bg-[#081626]">
  ...
  if (!me) {
    return (
      <div className="min-h-screen bg-[#081626] text-white flex flex-col items-center justify-center gap-4" dir="rtl">
        ...
        <Link to="/login" className="bg-[#009466] hover:bg-[#007a54] px-5 py-2 rounded-sm text-sm font-bold">تسجيل الدخول</Link>
      </div>
    );
  }
  ...
  if (me.role !== "admin") {
    return (
      <div className="min-h-screen bg-[#081626] text-white flex flex-col items-center justify-center gap-4" dir="rtl">
        ...
        <Link to="/" className="text-[#009466] text-sm font-bold hover:underline">العودة للموقع →</Link>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#081626] text-white" dir="rtl">
      <header className="border-b border-white/10 bg-[#0A1A30] sticky top-0 z-20">
        <div className="max-w-6xl mx-auto px-5 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="w-9 h-9 rounded-sm bg-[#D4A537]/15 flex items-center justify-center">
          ...
          <div className="flex items-center gap-3">
            <span className="text-[#D4A537] text-[10px] font-bold bg-[#D4A537]/10 border border-[#D4A537]/30 px-2.5 py-1 rounded-full">مشغّل</span>
      ...
      <main className="max-w-6xl mx-auto px-5 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 mb-8">
          <Stat icon={ClipboardCheck} label="إجمالي الطلبات" value={requests.length} color="#0696B0" />
          <Stat icon={Inbox} label="بانتظار الموافقة" value={pending.length} color="#D4A537" />
          <Stat icon={ClipboardCheck} label="عقود نشطة" value={accepted.length} color="#0696B0" />
          <Stat icon={ShieldCheck} label="تأجيرات مكتملة" value={completed.length} color="#009466" />
          <Stat icon={Building2} label="المزوّدون" value={providers.length} color="#009466" />
          <Stat icon={Truck} label="أسطول متاح" value={`${availableEq}/${equipment.length}`} color="#0696B0" />
        </div>

        <Tabs defaultValue="approvals" dir="rtl">
          <TabsList className="bg-white/5 border border-white/10 p-1 h-auto flex w-full mb-6">
            <TabsTrigger value="approvals" className="flex-1 data-[state=active]:bg-[#D4A537] data-[state=active]:text-[#0A1A30] text-white/60 text-sm gap-2">
            ...
            <TabsTrigger value="providers" className="flex-1 data-[state=active]:bg-[#D4A537] data-[state=active]:text-[#0A1A30] text-white/60 text-sm gap-2">
              <Building2 size={15} /> المزوّدون
            </TabsTrigger>
            <TabsTrigger value="fleet" className="flex-1 data-[state=active]:bg-[#D4A537] data-[state=active]:text-[#0A1A30] text-white/60 text-sm gap-2">
