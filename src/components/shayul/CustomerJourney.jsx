...
import { User, HardHat, Database, Shield } from "lucide-react";
    ...
    <section id="journey" className="py-24 bg-[#0A1A30] relative" dir={dir}>
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="flex flex-wrap gap-3 mb-14">
          ...
          <div className="flex items-center gap-2 bg-[#D4A537]/10 border border-[#D4A537]/30 rounded-full px-4 py-2">
            <Shield size={15} className="text-[#D4A537]" />
            <span className="text-[#D4A537] text-xs font-bold">{journey.insuranceSoon[lang]}</span>
          </div>
        </div>
            ...
            {journey.stages.map((stage) => (
              <div key={stage.num} className="relative ps-20 last:pb-0">
                ...
                <div className="bg-white/4 border border-white/10 rounded-sm p-5 hover:bg-white/[0.07] transition-colors">
                  ...
                  {stage.insuranceSoon && (
                    <div className="mt-3 inline-flex items-center gap-2 bg-[#D4A537]/10 border border-[#D4A537]/40 rounded-full px-3 py-1.5">
                      <Shield size={12} className="text-[#D4A537]" />
                      <span className="text-[#D4A537] text-xs font-bold">{journey.insuranceSoon[lang]}</span>
                    </div>
                  )}
                </div>
